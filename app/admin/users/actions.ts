...
type UserState = ActionState<
  ...
  | 'wage_type' | 'hourly_wage' | 'is_active' | 'email' | 'password'
>

/** zod 検証済みデータから DB INSERT/UPDATE 可能な列だけ抜き出す */
function pickInsertable(parsed: UserBase) {
  return {
    name: parsed.name,
    ...
    hourly_wage: parsed.hourly_wage ?? null,
...
export async function createUserAction(formData: FormData): Promise<UserState> {
  ...
  const { error: insertError } = await admin
    .from('users')
    .insert({ id: authData.user.id, ...pickInsertable(parsed.data) })
  if (insertError) {
...
export async function updateUserAction(
  ...
  const { error } = await supabase
    .from('users')
    .update(pickInsertable(parsed.data))
    .eq('id', userId)
